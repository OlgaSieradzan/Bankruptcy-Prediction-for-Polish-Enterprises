...
Rates$Accuracy <- as.numeric(Rates$Accuracy)
  ...
  ggplot(Rates[c(1,2,3,4,7,8,9,10,11),], aes(x = reorder(Method, Accuracy), y = Accuracy)) +
...
The QDA method has achived the highest accuracy of all of the models, but it is important to mention that differences between most of the models are not really significant. Definietly, altman model, tutson model and logistic regression with high treshhold, are the worst of all researched methods and can be defined as nor really promising. Machine learning algorithms performed disappointingly, it did not occur significantly better than statistical method as it may seem at first. 
...
Rates$`False Negative Rate` <- as.numeric(Rates$`False Negative Rate`)
  ...
  ggplot(Rates[c(1,2,3,4,7,8,9,10,11),], aes(x = reorder(Method, `False Negative Rate`), y = `False Negative Rate`)) +
  geom_bar(stat = "identity", fill = "orange") +
